...
function draw() {
  ...
  for (j = 0; j < height; j += height/lineNum) {
    ...
    // easing
    const firstPlusRatio = map(frameCount % (403/1), 0, 100/1, cycleLen/4, cycleLen/2);
    const firstPlusLinear = ease.linear(firstPlusRatio);
    const firstMinusRatio = map(frameCount % (403/1), 0, 100/1, -cycleLen/4, 0);
    const firstMinusLinear = ease.linear(firstMinusRatio);
    
    const secondPlusRatio = map(frameCount % (403/1), 0, 100/1, cycleLen/2, cycleLen/4);
    const secondPlusLinear = ease.linear(secondPlusRatio);
    const secondMinusRatio = map(frameCount % (403/1), 0, 100/1, 0, -cycleLen/4);
